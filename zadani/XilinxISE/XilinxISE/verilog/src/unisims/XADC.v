...
//		# ((time_out - prev_time_out) * 1000);
    ...
    $display("FORMAT: First line is header line. Valid column name are: TIME TEMP VCCINT VCCAUX VBRAM VP VN VAUXP[0] VAUXN[0] ..... \n");
...
// DRPORT - SRAM
    ...
    always @(posedge dclk_in or posedge gsr_in) 
     ...
     if (gsr_in == 1) begin
        ...
        if (den_in == 1 && (daddr_in >7'h5F || (daddr_in >= 7'h2E && daddr_in < 7'h3F)))
...
// DO bus data out
   ...
   always @( daddr_in_lat or  tmp_data_reg_out or tmp_dr_sram_out or status_reg ) begin
     ...
     if ((daddr_in_lat >= 7'h00  && daddr_in_lat <= 7'h2F)) 
...
// Conversion
   always @(adc_state or next_state or curr_chan  or mn_mux_in or curr_b_u) begin
	...
	if ((adc_state == S3_ST && next_state == S5_ST) ||  adc_state == S5_ST) begin
	    ...
	    else if (curr_chan == 1 || curr_chan == 2 || curr_chan ==6) begin     // internal power conversion
   ...
   always @(adc_state or next_state or curr_chan2  or mn_mux_in2 or curr_b_u2) begin
	...
	if ((adc_state == S3_ST && next_state == S5_ST) ||  adc_state == S5_ST) begin
	    ...
	    else if (curr_chan2 == 1 || curr_chan2 == 2 || curr_chan2 == 6) begin     // internal power conversion
    ...
    always @(posedge eoc_out_t or posedge rst_in_not_seq) 
     ...
     if (eoc_out == 1) begin
		if ((curr_chan_lat >= 0 && curr_chan_lat <= 3)  || (curr_chan_lat == 6) ||
           (curr_chan_lat >= 16 && curr_chan_lat <= 31)) begin
     ...
     if (eoc_out2 == 1) begin
		if ((curr_chan_lat2 >= 0 && curr_chan_lat2 <= 3)  || (curr_chan_lat2 == 6) ||
           (curr_chan_lat2 >= 16 && curr_chan_lat2 <= 31)) begin
...
// alarm
       ...
       if (rst_lock == 0) begin
        if (curr_chan_lat == 0) begin
	      if (data_written >= ot_limit_reg)
           ...
           ((curr_seq1_0_lat[3:2] == 2'b10 || curr_seq1_0_lat[3:2] == 2'b01 ||
             (curr_seq1_0_lat[3:2] == 2'b00 && curr_seq1_0_lat[1:0] != 2'b00)))) || 
             ((curr_seq1_0_lat ==4'b0000 || curr_seq1_0_lat[3:2] == 2'b11)
                 && (data_written < ot_sf_limit_low_reg)))
                ot_out_reg <= 0;
    ...
    always @(ot_out_reg or ot_en or alarm_out_reg or alarm_en)
       begin
             ...
             alarm_out[3:0] = alarm_out_reg[3:0] & alarm_en[3:0];
             alarm_out[7] = |alarm_out[3:0];
      end
