...
//  tri0 GSR, GTS, GWE;
  ...
  reg  [31:0] testmode_reg = 32'b0;
  reg  [31:0] memrd_param_reg = 32'b0;
  reg  [31:0] dwc_reg = 32'b0;
  reg  [31:0] trim_reg = 32'b0;
  reg  [31:0] bout_reg = 32'b0;
  reg  [31:0] bspi_reg = 32'h000B;
...
//    assign rw_en = (((icap_on == 0 || icap_init_done == 0) && buswid_flag == 1) || icap_init_done == 1) ? rw_en_tmp : 0;
      ...
      if (rst_intl == 0) begin
        ...
        if (new_data_in_flag == 1 && wr_flag == 1) begin
           if (conti_data_flag == 1 ) begin
               ...
               5'b00001 : far_reg <= {6'b0, pack_in_reg[25:0]};
               ...
               5'b01100 : begin
                          ...
                          if (pack_in_reg[27:0] != DEVICE_ID[27:0]) begin
                             id_error_flag <= 1;
                             if (icap_on == 0)
                             $display("Error : written value to IDCODE register is %h which does not match DEVICE ID %h on X_SIM_CONFIGE2 instance %m at time %t.", pack_in_reg, DEVICE_ID, $time);
                             else
                             $display("Error : written value to IDCODE register is %h which does not match DEVICE ID %h on X_ICAPE2 instance %m at time %t.", pack_in_reg, DEVICE_ID, $time);
                          end 
                          else
                             id_error_flag <= 0;
                          end
               ...
               5'b10111 : testmode_reg <= pack_in_reg;
               5'b11000 : ctl1_reg <= (pack_in_reg & mask_reg) | (ctl1_reg & ~mask_reg);
               5'b11001 : memrd_param_reg <= {4'b0,  pack_in_reg[27:0]};
               5'b11010 : dwc_reg <= {4'b0,  pack_in_reg[27:0]};
               5'b11011 : trim_reg <= pack_in_reg;
               5'b11110 : bout_reg <= pack_in_reg;
               5'b11111 : bspi_reg <= pack_in_reg;
               endcase
    ...
    if (rst_intl == 0) begin
        ...
        if (cmd_rd_flag == 1 && rdwr_b_in == 1 && csi_b_in == 0) begin
               ...
               5'b11000 : if (buswidth == 2'b01)
                             ...
                             rdbk_2wd(ctl1_reg);
               5'b11001 : if (buswidth == 2'b01)
                             rdbk_byte(memrd_param_reg);
                          else if (buswidth == 2'b10)
                             rdbk_wd(memrd_param_reg);
                          else if (buswidth == 2'b11)
                             rdbk_2wd(memrd_param_reg);
               5'b11010 : if (buswidth == 2'b01)
                             rdbk_byte( dwc_reg);
                          else if (buswidth == 2'b10)
                             rdbk_wd( dwc_reg);
                          else if (buswidth == 2'b11)
                             rdbk_2wd(dwc_reg);
               5'b11011 : if (buswidth == 2'b01)
                             rdbk_byte(trim_reg);
                          else if (buswidth == 2'b10)
                             rdbk_wd(trim_reg);
                          else if (buswidth == 2'b11)
                             rdbk_2wd(trim_reg);
               5'b11111 : if (buswidth == 2'b01)
                             rdbk_byte(bspi_reg);
                          else if (buswidth == 2'b10)
                             rdbk_wd(bspi_reg);
                          else if (buswidth == 2'b11)
   ...
   always @(posedge gsr_cmd_out or negedge  rw_en)
    ...
    assign stat_reg[31:27] = 5'b00000; 
